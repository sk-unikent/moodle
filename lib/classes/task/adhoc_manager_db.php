...
class adhoc_manager_db extends adhoc_manager {
     ...
     */
    public function queue_adhoc_task(adhoc_task $task, $checkforexisting = false) {
        global $DB;
        ...
        $record = $this->record_from_adhoc_task($task);
        ...
        if ($checkforexisting && $this->task_is_scheduled($task)) {
            return false;
        }
     ...
     * @return bool
     */
    protected function task_is_scheduled($task) {
        global $DB;
        $record = $this->record_from_adhoc_task($task);
     ...
     */
    public function get_adhoc_tasks($classname) {
        global $DB;
        ...
        return array_map(array($this, 'adhoc_task_from_record'), $records);
    }
     ...
     * @return \stdClass
     */
    public function record_from_adhoc_task($task) {
        $record = new \stdClass();
     ...
     * @return \core\task\adhoc_task
     */
    public function adhoc_task_from_record($record) {
    ...
    public function get_next_adhoc_task($timestart) {
        ...
        foreach ($records as $record) {
            if ($lock = $cronlockfactory->get_lock('adhoc_' . $record->id, 0)) {
                ...
                $task = $this->adhoc_task_from_record($record);
     ...
     * @param \core\task\adhoc_task $task
     */
    public function adhoc_task_delete(adhoc_task $task) {
        global $DB;
